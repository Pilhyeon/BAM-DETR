...
def compute_mr_r1(submission, ground_truth, iou_thds=np.linspace(0.5, 0.7, 3)):
    ...
    return iou_thd2recall_at_one, mIoU
...
def eval_moment_retrieval(submission, ground_truth, dataset, verbose=True):
    ...
    if dataset == "charades":
        ...
        length_ranges = [[0, 20], [20, 50], [50, 1000], [0, 1000], ]  #
    ...
    for l_range, name in zip(length_ranges, range_names):
        ...
        iou_thd2recall_at_one, mIoU = compute_mr_r1(_submission, _ground_truth)

        ret_metrics[name] = {"MR-mAP": iou_thd2average_precision, "MR-R1": iou_thd2recall_at_one, "MR-mIoU": float(f"{100 * mIoU:.2f}")}
        if verbose:
...
def eval_submission(submission, ground_truth, dataset, verbose=True, match_number=True):
    """
    Args:
    ...
    if "pred_relevant_windows" in submission[0]:
        moment_ret_scores = eval_moment_retrieval(
            submission, ground_truth, dataset, verbose=verbose)
