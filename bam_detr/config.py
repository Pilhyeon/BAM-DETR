...
class BaseOptions(object):
    ...
    def initialize(self):
        ...
        parser.add_argument("--lr_drop", type=int, default=80, help="drop learning rate to 1/10 every lr_drop epochs")
        parser.add_argument("--lr_gamma", type=float, default=0.1, help="lr reduces the gamma times after the `drop' epoch")
        parser.add_argument("--lr_warmup", type=float, default=0, help="linear warmup scheme")
        ...
        parser.add_argument("--quality_loss", action="store_true",
                            help="Disable quality_loss between matched query spans and the text.")
        ...
        parser.add_argument('--set_cost_class', default=0, type=float,
                            help="Class coefficient in the matching cost") # 4

        # * Loss coefficients
        parser.add_argument('--span_loss_coef', default=10, type=float)
        parser.add_argument('--giou_loss_coef', default=1, type=float) # 1
        parser.add_argument('--label_loss_coef', default=0, type=float) # 4
        parser.add_argument('--quality_loss_coef', default=2, type=float)
        parser.add_argument('--boundary_loss_coef', default=1, type=float) # 1
        ...
        parser.add_argument("--nms_thd", type=float, default=0.3,
        ...
        parser.add_argument("--stop_metrics", type=str, default="MR-full-mAP")
        self.parser = parser
    ...
    def parse(self, a_feat_dir=None):
        ...
        if isinstance(self, TestOptions):
            ...
            save_fns = ['bam_detr/model.py', 'bam_detr/transformer.py']
