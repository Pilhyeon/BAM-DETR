...
from bam_detr.config import TestOptions
from bam_detr.model import build_model
from bam_detr.span_utils import span_cxw_to_xx
from bam_detr.start_end_dataset import StartEndDataset, start_end_collate, prepare_batch_inputs
from bam_detr.start_end_dataset_audio import \
    StartEndDataset_audio, start_end_collate_audio, prepare_batch_inputs_audio
from bam_detr.postprocessing import PostProcessorDETR
from standalone_eval.eval import eval_submission
from utils.basic_utils import save_jsonl, save_json
from utils.temporal_nms import temporal_nms

from bam_detr.misc import WarmupStepLR
    ...
    if opt.nms_thd == -1:
        if opt.eval_split_name in ["val", "test"]:  # since test_public has no GT
            metrics = eval_submission(
                submission, gt_data, opt.dset_name,
        ...
        submission_nms_path = submission_path.replace(".jsonl", "_nms.jsonl")
        save_jsonl(submission_after_nms, submission_nms_path)
        
        if opt.eval_split_name == "val":
            metrics = eval_submission(
                submission_after_nms, gt_data, opt.dset_name,
                verbose=opt.debug, match_number=not opt.debug
            )
            
            save_metrics_nms_path = submission_path.replace(".jsonl", "_metrics.json")
            save_json(metrics, save_metrics_nms_path, save_pretty=True, sort_keys=False)
            latest_file_paths = [submission_path, save_metrics_nms_path, ]
        else:
            metrics = None
            latest_file_paths = [save_metrics_path, ]
...
def compute_mr_results(model, eval_loader, opt, epoch_i=None, criterion=None, tb_writer=None):
    ...
    for batch in tqdm(eval_loader, desc="compute st ed scores"):
        ...
        if opt.quality_loss:
            prob = outputs["pred_quality"].repeat(1, 1, 2)

        outputs["scores"] = prob[..., 0]
...
def setup_model(opt):
    ...
    if opt.lr_warmup != -1 and opt.lr_drop > 0:
        lr_scheduler = WarmupStepLR(optimizer, warmup_steps=opt.lr_warmup, step_size=opt.lr_drop, gamma=opt.lr_gamma)
    elif opt.lr_warmup != -1:
        from transformers import get_constant_schedule_with_warmup
        lr_scheduler =  get_constant_schedule_with_warmup(optimizer, opt.lr_warmup)
    else:
...
def start_inference(train_opt=None, split=None, splitfile=None):
    ...
    if opt.eval_split_name == 'val':
        logger.info("metrics {}".format(pprint.pformat(metrics_no_nms["brief"], indent=4)))

from sys import argv
if __name__ == '__main__':
    split, splitfile = argv[4], argv[6]

    if any(['debug' in item for item in argv]):  # keep the model run deterministically
