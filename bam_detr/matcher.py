...
from bam_detr.span_utils import generalized_temporal_iou, span_cxw_to_xx
...
class HungarianMatcher(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, outputs, targets, key=None):
        ...
        bs, num_queries = outputs[key].shape[:2]
        ...
        if self.span_loss_type == "l1":
            ...
            out_spans = outputs[key].flatten(0, 1)  # [batch_size * num_queries, 2]

            cost_span = torch.cdist(span_cxw_to_xx(out_spans), span_cxw_to_xx(tgt_spans), p=1)  # [batch_size * num_queries, total #spans in the batch]
            ...
            pred_spans = outputs[key]  # (bsz, #queries, max_v_l * 2)
